...
def post_ads(request):
    if request.method == 'POST':
        ...
        mail_subject = "[SITE TESTE] - Novo anúncio"
        sender_email = request.user.email
        message = f"Prezado Admin, você recebeu um novo pedido de anúncio de {sender_email}"
        print(message)
...
def ads_search(request):
    ...
    search_string = request.GET.get('customword')

    if search_string:
        ads_search_result = Ads.objects.filter(description__contains=search_string)
    elif state:
